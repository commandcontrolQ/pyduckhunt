# Python Arcade-style Game!
import pygame, math
from os.path import exists 

"""
The font used and the music used is different from the original.
To change the font, refer to line 27.
(THE FONT MUST BE IN TTF FORMAT + MUST BE IN "assets/font/" folder)
You will also need to change the font size in lines 28 and 29, 
as well as when they are used in drawMenu() and drawEnd(),
as the height parameter has been changed to suit the font.

To change the music, refer to lines 84-89.
(MUST INCLUDE EXTENSION)
If line 84 is changed so that 'musicFile' no longer uses an input statement,
then pygame will always refer to the specified file when starting the game.
If you want to permanently set the music to a specific file,
- Remove lines 84 to 88
- Revert the string in line 89 {pygame.mixer.music.load(...)} from a fast-string to a regular string (remove the beginning f)
- Replace the brackets in line 89 (should appear as '{musicFile}') with the name and extension of the music file
...
fontName = "pdos_windows"
font = pygame.font.Font(f'assets/font/{fontName}.ttf', 23)
fontBig = pygame.font.Font(f'assets/font/{fontName}.ttf', 60)
width = 900
height = 800
...
over_img = pygame.image.load(f'assets/menus/gameOver.png')
...
pygame.mixer.init()
musicFile = input("Enter name & extension of music file: ")
if exists(f"assets/sounds/{musicFile}"):
    pass
else:
    musicFile = "bg_music.mp3"
pygame.mixer.music.load(f'assets/sounds/{musicFile}')
soundPlate = pygame.mixer.Sound('assets/sounds/Broken plates.wav')
soundPlate.set_volume(.2)
soundBird = pygame.mixer.Sound('assets/sounds/Drill Gear.mp3')
soundBird.set_volume(.2)
soundLaser = pygame.mixer.Sound('assets/sounds/Laser Gun.wav')
soundLaser.set_volume(.3)
...
def drawScore():
    ...
    if mode == 1:
        mode_text = font.render(f'Remaining bullets: {ammo}', True, 'black')
    if mode == 2:
        mode_text = font.render(f'Remaining time: {time_remaining}', True, 'black')
...
def drawMenu():
    ...
    screen.blit(font.render(f'{best_freeplay}', True, 'black'), (340, 580 +9))
    ammo_button = pygame.rect.Rect((475, 524), (260, 100))
    screen.blit(font.render(f'{best_ammo}', True, 'black'), (650, 580 +9))
    timed_button = pygame.rect.Rect((170, 661), (260, 100))
    screen.blit(font.render(f'{best_timed}', True, 'black'), (350, 710 +9))
    ...
    if timed_button.collidepoint(mouse_pos) and clicks[0] and not clicked:
        ...
        level = 1
        menu = False
        time_remaining = 45 # originally 30
        time_passed = 0
        total_shots = 0
        points = 0
        clicked = True
...
def drawEnd():
    ...
    screen.blit(fontBig.render(f'{display_score}', True, 'black'), (650, 570 +11))
...
while run:
    ...
    if level == 1:
        target_boxes = drawLevel(one_coords)
        one_coords = moveLevel(one_coords)
        if shot:
            one_coords = shotCheck(target_boxes, one_coords)
            shot = False
    elif level == 2:
        target_boxes = drawLevel(two_coords)
        two_coords = moveLevel(two_coords)
        if shot:
            two_coords = shotCheck(target_boxes, two_coords)
            shot = False
    elif level == 3:
        target_boxes = drawLevel(three_coords)
        three_coords = moveLevel(three_coords)
        if shot:
            three_coords = shotCheck(target_boxes, three_coords)
            shot = False
    if level > 0:
        drawGun()
        drawScore()
